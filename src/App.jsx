...
export default function App() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-[#FDFAF6] header-meander flex flex-col items-center justify-center px-4">
      ...
      {emailSent ? (
        <p className="text-[#5C4A3A] text-center max-w-xs">Check your email for a magic link to sign in.</p>
      ) : (
        <form onSubmit={signIn} className="w-full max-w-sm flex flex-col gap-3">
          ...
          />
          <button type="submit" className="w-full py-2 bg-[#C4613A] text-white text-sm font-medium rounded-lg hover:bg-[#A8522F] transition-colors">
            Send magic link
          </button>
        </form>
      )}
    ...
    <div className="min-h-screen bg-[#FDFAF6]">
      <div className="header-meander bg-[#FDFAF6] border-b border-[#E8DDD0]">
        <div className="max-w-3xl mx-auto px-4 pt-8 pb-6 text-center relative">
          <button onClick={signOut} className="absolute top-0 right-0 text-xs text-[#8B7355] hover:text-[#5C4A3A] transition-colors">
            Sign out
          </button>
          ...
          <div className="flex">
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors ${
                activeTab === 'lessons' ? 'border-[#C4613A] text-[#C4613A]' : 'border-transparent text-[#8B7355] hover:text-[#5C4A3A]'
              }`}
            >
              Lessons
            </button>
            <button
              ...
              className={`px-5 py-3 text-sm font-semibold border-b-2 transition-colors ${
                activeTab === 'flashcards' ? 'border-[#C4613A] text-[#C4613A]' : 'border-transparent text-[#8B7355] hover:text-[#5C4A3A]'
              }`}
            >
              Flashcards
            </button>
                ...
                {LEVELS.map(level => (
                  <button
                    ...
                    className={`px-4 py-2.5 sm:px-3 sm:py-1 rounded-full text-xs font-semibold transition-colors ${
                      activeLevel === level ? 'bg-[#C4613A] text-white' : 'bg-[#F0EAE0] text-[#5C4A3A] hover:bg-[#E8DDD0]'
                    }`}
                  >
                    {level}
                  </button>
          ...
          {selected && !loading && (
            <div className="space-y-10">
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {completedLessons.has(selected.id) && (
                      <span className="px-3 py-1.5 rounded-full text-sm font-bold bg-[#E8F5E9] text-green-700 border border-green-200">✓ Completed</span>
                    )}
                    {selected.level && (
                      <span className="px-3 py-1.5 rounded-full text-sm font-bold bg-[#F0E4D8] text-[#C4613A] border border-[#E8C9B0]">{selected.level}</span>
                    )}
                  </div>
                </div>
                {selected.focus && <p className="mt-3 text-[#5C4A3A] text-sm">{selected.focus}</p>}
              ...
              {(selected.grammar_title || grammar.length > 0) && (
                <section>
                  <h2 className="text-lg font-semibold text-[#5C4A3A] border-b border-[#E8DDD0] pb-1 mb-3">Grammar</h2>
                  {selected.grammar_title && <p className="font-medium text-[#2C1810] mb-1">{selected.grammar_title}</p>}
                  {selected.grammar_rule_summary && <p className="text-[#5C4A3A] text-sm mb-4">{selected.grammar_rule_summary}</p>}
                  {grammar.length > 0 && (
                    <div className="space-y-3">
                      {grammar.map(gp => (
                        <div key={gp.id} className="bg-[#FFFCF8] border border-[#E8DDD0] rounded-lg px-4 py-3 shadow-[0_1px_6px_rgba(139,99,60,0.10)]">
                          ...
                          {gp.romanization && <p className="text-[#8B7355] text-sm italic">{gp.romanization}</p>}
                          {gp.note && <p className="text-[#5C4A3A] text-sm mt-1">{gp.note}</p>}
                        </div>
                      ))}
                    </div>
                  )}
              ...
              {vocabulary.length > 0 && (
                <section>
                  <h2 className="text-lg font-semibold text-[#5C4A3A] border-b border-[#E8DDD0] pb-1 mb-3">Vocabulary</h2>
                    ...
                    {vocabulary.map(v => (
                      <div key={v.id} className="bg-[#FFFCF8] px-4 py-3 grid grid-cols-2 gap-4">
                        <div>
                          ...
                          {v.pronunciation && <p className="text-xs text-[#8B7355] italic">{v.pronunciation}</p>}
                          {v.part_of_speech && <p className="text-xs text-[#6B7C3A] mt-0.5">{v.part_of_speech}</p>}
                        </div>
                        <div>
                          <p className="text-[#3D2E1E]">{v.translation}</p>
                          {v.example_sentence && <p className="text-xs text-[#8B7355] mt-1 italic">{v.example_sentence}</p>}
                          {v.example_translation && <p className="text-xs text-[#8B7355]">{v.example_translation}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {passages.length > 0 && (
                <section>
                  <h2 className="text-lg font-semibold text-[#5C4A3A] border-b border-[#E8DDD0] pb-1 mb-3">Passage</h2>
                  <div className="space-y-4">
                    {passages.map(p => (
                      <div key={p.id} className="bg-[#FFFCF8] border border-[#E8DDD0] rounded-lg px-4 py-4 shadow-[0_1px_6px_rgba(139,99,60,0.10)]">
                        {p.passage_title && <p className="font-medium text-[#2C1810] mb-2">{p.passage_title}</p>}
                        <p className="font-serif text-[#2C1810] leading-relaxed">{p.greek_text}</p>
                        {p.annotation_note && <p className="text-xs text-[#8B7355] mt-2 italic">{p.annotation_note}</p>}
                        {p.english_translation && (
                          <div className="mt-3">
                            <button onClick={() => toggleTranslation(p.id)} className="text-xs text-[#6B7C3A] hover:text-[#4A5A28] underline underline-offset-2">
                              {showTranslation[p.id] ? 'Hide translation' : 'Show translation'}
                            </button>
                            {showTranslation[p.id] && <p className="mt-2 text-sm text-[#5C4A3A] italic">{p.english_translation}</p>}
                          </div>
                        )}
                      </div>
                    ))}
              ...
              {exercises.length > 0 && (
                <section>
                  <h2 className="text-lg font-semibold text-[#5C4A3A] border-b border-[#E8DDD0] pb-1 mb-3">Exercises</h2>
                  <div className="space-y-4">
                    {exercises.map((ex, i) => {
                        ...
                        <div key={ex.id} className="bg-[#FFFCF8] border border-[#E8DDD0] rounded-lg px-4 py-4 shadow-[0_1px_6px_rgba(139,99,60,0.10)]">
                          <div className="mb-3">
                            {(() => {
                              ...
                              return match ? (
                                <>
                                  <p className="text-[#2C1810]"><span className="text-[#B8A898] text-sm mr-2">{i + 1}.</span>{match[1]}</p>
                                  <p className="text-[#8B7355] text-sm mt-0.5 ml-5">{match[2]}</p>
                                </>
                              ) : (
                                <p className="text-[#2C1810]"><span className="text-[#B8A898] text-sm mr-2">{i + 1}.</span>{ex.question}</p>
                              )
                            })()}
                          </div>
                          <div className="flex flex-col gap-2">
                            <input
                              ...
                              className={`w-full border rounded-lg px-3 py-2 text-sm outline-none transition-colors ${
                                result === true ? 'border-green-400 bg-green-50' : result === false ? 'border-red-400 bg-red-50' : 'border-[#D4C5B0] focus:border-[#C4613A]'
                              }`}
                            />
                            <div className="flex gap-2">
                              <button onClick={() => checkAnswer(ex)} className="px-4 py-2 bg-[#C4613A] text-white text-sm font-medium rounded-lg hover:bg-[#A8522F] transition-colors">Check</button>
                              <button onClick={() => setShownAnswers(a => ({ ...a, [ex.id]: !a[ex.id] }))} className="px-4 py-2 bg-[#F0EAE0] text-[#5C4A3A] text-sm font-medium rounded-lg hover:bg-[#E8DDD0] transition-colors">
                                {shownAnswers[ex.id] ? 'Hide' : 'Show Answer'}
                              </button>
                            </div>
                          </div>
                          {shownAnswers[ex.id] && (
                            <p className="mt-2 text-sm text-[#8B7355]">Answer: <span className="font-medium text-[#3D2E1E]">{ex.correct_answer}</span></p>
                          )}
                          {result === true && <p className="mt-2 text-sm text-green-600 font-medium">Correct!</p>}
                          {result === false && (
                            <div className="mt-2">
                              <p className="text-sm text-red-500 font-medium">Try again</p>
                              {ex.hint_text && <p className="text-xs text-[#8B7355] mt-0.5">Hint: {ex.hint_text}</p>}
                            </div>
                          )}
                        </div>
                      )
